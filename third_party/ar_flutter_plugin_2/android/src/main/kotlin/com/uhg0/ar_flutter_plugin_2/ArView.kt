...
import com.uhg0.ar_flutter_plugin_2.Serialization.serializePose
...
class ArView(
    ...
    private suspend fun buildModelNode(nodeData: Map<String, Any>): ModelNode? {
        ...
        val nodeType = nodeData["type"] as? Int ?: return null
        ...
        return try {
            val modelInstance = when (nodeType) {
                0 -> { // GLTF2/GLB model from Flutter asset folder.
                ...
                2, 3 -> { // GLB/GLTF2 model from the app documents folder.
                    val modelFile = File(fileLocation)
                    val resolvedFile = if (modelFile.isAbsolute) {
                        modelFile
                    } else {
                        File(viewContext.applicationInfo.dataDir, "app_flutter/$fileLocation")
                    }
                    if (!resolvedFile.exists()) {
                        Log.e(TAG, "Model file not found: ${resolvedFile.absolutePath}")
                        return null
                    }
                    sceneView.modelLoader.createModelInstance(resolvedFile)
                }
                else -> return null
            }
            ...
            Log.e(TAG, "Error loading model ${nodeData["uri"]}", e)
            null
        }
    }
                    ...
                    onSingleTapConfirmed = { motionEvent: MotionEvent, node: Node? ->
                        if (node != null) {
                            ...
                            while (currentNode != null) {
                                nodesMap.forEach { (name, modelNode) ->
                            ...
                            if(handleTaps) {
                                objectChannel.invokeMethod("onNodeTap", listOf(nodeName ?: node.name))
                            }
                            true
                        } else {
                            session?.update()?.let { frame ->
    ...
    private fun handleGetAnchorPose(
        ...
        try {
            ...
            val anchor = anchorNodesMap[anchorId]?.anchor
                ?: sceneView.session?.allAnchors?.find { it.cloudAnchorId == anchorId }
            if (anchor != null) {
                result.success(serializePose(anchor.pose).toList())
            } else {
                result.error("ANCHOR_NOT_FOUND", "Anchor with ID $anchorId not found", null)
            }
        } catch (e: Exception) {
